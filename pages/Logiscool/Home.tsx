          ...
          <div className="flex flex-1 justify-end gap-8">
            <div className="flex items-center gap-9">
              {programCategories.map((item) => (
                <a key={item.title} className="text-[#181015] text-sm font-medium leading-normal hover:underline" href="#">
                  {item.title}
                </a>
              ))}
            </div>
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(158px,1fr))] gap-6 p-4">
              {programCategories.map((item) => (
                <div key={item.title} className="flex flex-col gap-3 pb-3">
                  ...
                  <button className="flex min-w-full cursor-pointer items-center justify-center rounded-lg h-10 px-4 bg-[#a80057] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#900047] transition-colors">Explore</button>
                </div>
              ))}
            </div>
          </div>
