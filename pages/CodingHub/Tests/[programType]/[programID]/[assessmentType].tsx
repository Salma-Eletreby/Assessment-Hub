...
const AssessmentPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch("/api/codinghub/programs")
      .then((res) => res.json())
      .then((data) => {
        const program = data.find((p: any) => p.id === programID);
        if (!program) return;
        setCampName(program.name);
        const assessment = program.assesments.find((a: any) => a.type === assessmentType);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-[#00426b]">
      <Head>
      ...
      <main id="assessment-main" className="flex-grow p-6 max-w-3xl mx-auto space-y-6">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-center gap-10">
          <div className="flex flex-col">
            ...
            <input id="studentName" type="text" placeholder="Enter your name" className="px-3 py-2 border border-[#00426b] rounded bg-[#FFF5F2] focus:outline-none focus:ring-2 focus:ring-[#568F87]" />
          </div>
          <div className="flex flex-col">
            <label htmlFor="assessmentDate" className="font-medium text-sm mb-1">
              Date
            </label>
            <input id="assessmentDate" type="date" defaultValue={new Date().toISOString().split("T")[0]} className="px-3 py-2 border border-[#00426b] rounded bg-[#FFF5F2] focus:outline-none focus:ring-2 focus:ring-[#568F87]" />
          </div>
        </div>
        {questions.map((q, idx) => {
          const userAnswer = answers[idx];
          switch (q.type) {
            case "mcq":
              return (
                <div key={idx} className="p-4 rounded-lg bg-[#ecf4f9] shadow-md">
        ...
        <div className="flex justify-center mt-6">
          <button onClick={handleSubmit} className="px-6 py-3 bg-[#0077a8] text-white font-bold rounded-lg hover:bg-[#40b5ad] transition-transform transform hover:scale-110 hover:shadow-xl duration-300 ease-in-out">
            Submit
          </button>
        </div>
      </main>{" "}
